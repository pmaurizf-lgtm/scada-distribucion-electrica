/**
 * Explora filas Excel relacionadas con LCS-4PWS0003 / TRF-6PWS0003.
 */
...
function dumpHits(label, rows) {
  ...
  for (const [rn, row] of rows) {
    ...
    if (!/LCS-4PWS0003|TRF-6PWS0003|CENTRO DE CARGA N-3|TRANSFORMADOR N[º°]?3/i.test(blob))
      continue
    const o = row.E
    if (o === 'TRF-6PWS0003') fromTrf++
    if (o === 'LCS-4PWS0003') fromLcs++
    hits.push({
      rn,
      D: row.D,
      E: row.E,
  ...
  console.log(label, { total: hits.length, fromTrf, fromLcs })
...
for (const [, row] of report) {
  if (row.B === 'LCS-4PWS0003' || row.B === 'TRF-6PWS0003') {
    console.log('equip', {
      B: row.B,
