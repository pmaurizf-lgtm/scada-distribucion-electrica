/**
 * Importa alimentaciones 230 V aguas abajo de LCS-4PWS0001 (Excel «230V Power System»).
 *
 * - BFS desde salidas 230 del LCS-0001 ya presentes en abtDownstream.json
 * - Para cada SSB con Incoming Power Switch: circuito INS + barra virtual
 * - Salidas SSB / cargas con Breaker ID / modelo Excel
 *
 * Uso:
 *   node scripts/import-lcs0001-downstream-230.mjs [LCS-4PWS000N]
 *   node scripts/import-lcs0001-downstream-230.mjs LCS-4PWS0002
 */
...
if (!/^LCS-4PWS000[1-6]$/.test(LCS)) {
  console.error('Uso: node scripts/import-lcs0001-downstream-230.mjs LCS-4PWS000N')
  process.exit(1)
}
const INS_NOTE = 'ssb-incoming'
const VOLTAGE = '230'
const ID_PREFIX = 'abt-230'
...
const rows = parseSheet(join(base, 'worksheets/sheet3.xml'))
  ...
  file.circuits
    .filter(
      (c) =>
        c.originId === LCS &&
        !c.virtual &&
        String(c.voltage).startsWith('230') &&
...
console.log(`Seed ${LCS} 230 outlets:`, [...seed].sort().join(', ') || '(none)')
...
for (const c of file.circuits) {
  const m = String(c.id).match(/^abt-230-(\d+)$/)
...
file._comment =
  (file._comment || '') +
  ` | Downstream 230 ${LCS} (+INS SSB) ${new Date().toISOString().slice(0, 10)}`
