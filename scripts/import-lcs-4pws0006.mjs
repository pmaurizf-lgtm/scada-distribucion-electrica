/**
 * Importa cadena ABT-6PWS0006 → TRF-6PWS0006 → LCS-4PWS0006
...
const LCS = 'LCS-4PWS0006'
const TRF = 'TRF-6PWS0006'
const ABT = 'ABT-6PWS0006'
...
/** Excel a veces añade " - [PB1]" u otros sufijos al Puma ID. */
function normalizePumaId(v) {
  const s = str(v)
  if (!s || !isPuma(s)) return s
  const m = s.match(/^[A-Z]{2,}-[A-Z0-9]+/i)
  return m ? m[0] : s.split(/\s+-\s+/)[0].trim()
}

function isRespeto(dest, desc) {
  const id = normalizePumaId(dest) ?? dest
  if (!isPuma(id)) return true
...
function extractVoltage(rows, voltage, localMap, startId) {
  ...
  for (const [rn, row] of [...rows.entries()].sort((a, b) => a[0] - b[0])) {
    ...
    } else if (isPuma(String(destRaw ?? ''))) {
      destId = normalizePumaId(destRaw)
    } else {
      const prot = protectionFromRow(row, circuitRef)
      destId = `SPARE-${LCS}-${prot.replace(/[^A-Za-z0-9-]/g, '')}`
    }
    ...
    let notes =
      mCol === 'Alternative' || mCol === 'Normal' || mCol === 'Alternativa'
        ? `Col. M Excel: ${mCol}`
        : null
    if (isQsParallel && fromLcs && isPuma(String(destRaw ?? '')) && !spare) {
      const csbId = normalizePumaId(destRaw)
...
const r440 = extractVoltage(s440, '440', localMap, 501)
...
if (!data.chains.some((c) => c.loadCenterId === LCS)) {
  data.chains.push({
    ...
    transformerId: TRF,
    loadCenterId: LCS,
    transformerName: 'TRANSFORMADOR Nº6 690V/440V-230V',
    loadCenterName: 'CENTRO DE CARGA N-6',
    ratings: {
      kVA: '660 / 570 / 90',
      primaryV: '690',
      secondary440V: '440',
...
data._comment =
  'Cadenas ABT-6PWS0001/0002/0003/0004/0005/0006 → TRF → LCS-4PWS0001/0002/0003/0004/0005/0006 desde Excel 440V/230V Power System. Devanados TRF virtuales; QVS-440/230 entradas.'
