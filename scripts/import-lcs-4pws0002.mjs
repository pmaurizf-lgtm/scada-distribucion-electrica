/**
 * Importa cadena ABT-6PWS0002 → TRF-6PWS0002 → LCS-4PWS0002
 * con el mismo criterio que LCS-4PWS0001.
 */
...
const LCS = 'LCS-4PWS0002'
const TRF = 'TRF-6PWS0002'
const ABT = 'ABT-6PWS0002'
...
function protectionFromRow(row, circuitRef) {
  ...
  // Sufijo tras LCS-…-  (QVS-440, 1Q01, QVM_440…)
...
function kindOf(destId, desc, voltage) {
  ...
  if (destId.startsWith('SSB-') || destId.startsWith('ABT-') || destId.startsWith('LCS-'))
...
/** Locals desde Main Equipment Report */
...
/**
 * Cable section: en LCS-0001 era mm² numérico.
 * Buscar columna con sección: a menudo Y o similar. Inspeccionar headers.
 */
...
function extractVoltage(rows, voltage, localMap, startId) {
  ...
  for (const [rn, row] of [...rows.entries()].sort((a, b) => a[0] - b[0])) {
    ...
    // Filas de la cadena TRF↔LCS o salidas LCS
    const fromTrf =
      origin === TRF &&
      (String(destRaw) === LCS ||
    ...
    // Servicio: Excel a veces marca VS en acopladores; forzar VM/NV
    let service = serviceOf(row.N)
    if (isQvm) service = 'VM'
    if (isQnv) service = 'NV'
    if (isQvs) service = 'VS'

    // Bus virtual para acopladores (nunca SPARE)
    ...
    const spareFlag = isSection ? false : spare
    ...
    if (!equipment.has(destId) && destId !== LCS) {
      ...
      equipment.set(destId, {
        ...
        kind: kindOf(destId, destDesc, voltage),
        voltage: String(voltage),
...
const r440 = extractVoltage(s440, '440', localMap, 101)
...
if (!data.chains.some((c) => c.loadCenterId === LCS)) {
  data.chains.push({
    ...
    transformerId: TRF,
    loadCenterId: LCS,
    transformerName: 'TRANSFORMADOR Nº2 690V/440V-230V',
    loadCenterName: 'CENTRO DE CARGA N-2',
    ratings: {
      kVA: '660 / 570 / 90',
      primaryV: '690',
      secondary440V: '440',
...
data._comment =
  'Cadenas ABT-6PWS0001/0002 → TRF → LCS-4PWS0001/0002 desde Excel 440V/230V Power System. Devanados TRF virtuales; QVS-440/230 entradas.'
...
for (const eq of allEq) {
  if (haveEq.has(eq.id)) {
    // Actualizar local/name si faltaba
...
console.log(
  'new LCS circuits',
  data.circuits.filter((c) => c.id.includes('abt-440-1') || c.id.includes('abt-230-1') || (c.originId === LCS || c.destinationId === LCS)).length,
)
