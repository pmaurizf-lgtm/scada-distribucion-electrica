/**
 * Repara acopladores QVM/QNV de LCS-4PWS0002 (servicio VM/NV, no spare).
 * Alinea con el criterio de LCS-4PWS0001.
 */
...
for (const c of data.circuits) {
  ...
  const isQvm = /^QVM/i.test(c.protectionName) || /QVM/i.test(c.circuitRef || '')
  const isQnv = /^QNV/i.test(c.protectionName) || /QNV/i.test(c.circuitRef || '')
