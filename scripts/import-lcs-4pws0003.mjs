/**
 * Importa cadena ABT-6PWS0003 → TRF-6PWS0003 → LCS-4PWS0003
...
const r440 = extractVoltage(s440, '440', localMap, 201)
...
if (!data.chains.some((c) => c.loadCenterId === LCS)) {
  data.chains.push({
    ...
    transformerId: TRF,
    loadCenterId: LCS,
    transformerName: 'TRANSFORMADOR Nº3 690V/440V-230V',
    loadCenterName: 'CENTRO DE CARGA N-3',
    ratings: {
      kVA: '660 / 570 / 90',
      primaryV: '690',
      secondary440V: '440',
...
data._comment =
  'Cadenas ABT-6PWS0001/0002/0003 → TRF → LCS-4PWS0001/0002/0003 desde Excel 440V/230V Power System. Devanados TRF virtuales; QVS-440/230 entradas.'
